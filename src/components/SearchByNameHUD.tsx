...
export const SearchByNameHUD = ({ open, onClose, onSelect }: SearchByNameHUDProps) => {
  const { catalog, products: productList } = useStore();
  ...
  const results = useMemo(() => {
    ...
    const allItemsMap = new Map<string, CatalogItem>();
    ...
    return sortSearchResults(combinedItems, term).slice(0, 20);
  }, [term, catalog, productList]);
